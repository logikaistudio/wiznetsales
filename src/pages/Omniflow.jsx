...
import {
    ...
    LayoutDashboard, RefreshCw, Filter, CheckCircle
} from 'lucide-react';
...
const Omniflow = () => {
    ...
    return (
        <div className="h-[calc(100vh-64px)] flex flex-col bg-gray-100 overflow-hidden relative">

            {/* PERSISTENT HEADER WITH STATS */}
            <div className="bg-white border-b border-gray-200 px-4 py-2 flex items-center justify-between shrink-0 shadow-sm z-20">
                <div className="flex items-center gap-6">
                    <div
                        className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-1.5 rounded transition-colors"
                        onClick={() => { setSelectedChat(null); setViewMode('dashboard'); }}
                        title="Go to Dashboard"
                    >
                        <div className="bg-primary/10 p-1.5 rounded-lg">
                            <LayoutDashboard className="w-5 h-5 text-primary" />
                        </div>
                        <h2 className="font-bold text-gray-800 text-sm hidden md:block">Omniflow Control</h2>
                    </div>
                ...
                <div className="flex items-center gap-2">
                    <Button size="xs" variant="ghost" onClick={fetchTickets} title="Refresh Data">
                    ...
                    {viewMode === 'dashboard' ? (
                        <div className="flex-1 bg-gray-50 overflow-y-auto p-6 md:p-8">
                            <div className="max-w-4xl mx-auto">
                                <h2 className="text-lg font-bold text-gray-900 mb-4 bg-white p-4 rounded-lg shadow-sm border border-gray-200">
                                ...
                                <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                                    <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                                        <h3 className="font-bold text-gray-800 text-sm">Latest Tickets</h3>
                                    </div>
                                    <div className="divide-y divide-gray-100">
                                        {allTickets.length > 0 ? allTickets.slice(0, 8).map(ticket => (
                                            <div key={ticket.id} className="p-4 hover:bg-gray-50 flex items-center justify-between group cursor-pointer">
                ...
                <div className="w-96 bg-white border-l border-gray-200 flex flex-col shrink-0 overflow-hidden">
                    {customerData ? (
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center p-6 text-center text-gray-500">
                            {/* Only show prompt if not in dashboard mode, because dashboard is now self-explaining */}
                            {viewMode === 'dashboard' ? (
                                // Context Panel is Empty in Dashboard Mode (Optional: Show more stats here?)
                                <>
